...
module.exports = {
    interpret: function (args) {
        var config, reporter, ignore, ignoreFile,
        ...
        if (path.existsSync(projectIgnore)) {
            ignoreFile = projectIgnore;
        } else if (path.existsSync(defaultIgnore)) {
            ignoreFile = defaultIgnore
        }
        
        if (ignoreFile) {
            ignore = fs.readFileSync(ignoreFile, "utf-8").split("\n").map(function (line) {
                return line.trim();
